...
class DecoderLayer_QFS(nn.Module):
    def __init__(self, config: BartConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.self_attn_layer_norm = LayerNorm(self.embed_dim)
        self.encoder_attn = Attention_QFS(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            encoder_decoder_attention=True,
    ...
    def forward(
        ...
        x,
        encoder_hidden_states,
        encoder_answer_relevance_atten=None, # the answer relevance attention that will be incorporated in the cross
        encoder_attn_mask=None,
        layer_state=None,
        causal_mask=None,
        decoder_padding_mask=None,
        ...
        residual = x
        print("before self attention the size is{}".format(x.size())) # [1, 6, 1024]
        ...
        # Cross attention
        print("before cross attention the size is{}".format(x.size())) # [1, 6, 1024]
        print("the encoder_hidden_states size is {}".format(encoder_hidden_states.size()))
        residual = x
        assert self.encoder_attn.cache_key != self.self_attn.cache_key
        if self.normalize_before:
            x = self.encoder_attn_layer_norm(x)
...
class BartDecoder_QFS(nn.Module):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer
    is a :class:`DecoderLayer`.
    Args:
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        self.layers = nn.ModuleList(
            [DecoderLayer_QFS(config) for _ in range(config.decoder_layers)]
        )  # type: List[DecoderLayer]
        self.layernorm_embedding = LayerNorm(config.d_model) if config.normalize_embedding else nn.Identity()
        self.layer_norm = LayerNorm(config.d_model) if config.add_final_layer_norm else None
    ...
    def forward(
        ...
        **unused,
        ...
        # encoder_answer_relevance_atten = encoder_answer_relevance_atten.transpose(0, 1)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, layer_self_attn, layer_past = decoder_layer(
                x,
                encoder_hidden_states,
                encoder_answer_relevance_atten,
                encoder_attn_mask=encoder_padding_mask,
                decoder_padding_mask=decoder_padding_mask,
                layer_state=layer_state,
                causal_mask=decoder_causal_mask,
...
class Attention_QFS(nn.Module):
    ...
    def forward(
        ...
        attn_mask: Optional[Tensor] = None,
        output_attentions=False,
        encoder_answer_relevance_atten: Optional[Tensor] = None, # we add this to incorprate the answer relevance score from QA model into the generation process.
    ) -> Tuple[Tensor, Optional[Tensor]]:
        """Input shape: Time(SeqLen) x Batch x Channel"""
        static_kv: bool = self.encoder_decoder_attention
        tgt_len, bsz, embed_dim = query.size()
...
class BartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        self.decoder = BartDecoder_QFS(config, self.shared)
    ...
    def forward(
        ...
        encoder_answer_relevance_atten=None,
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, past, attention_mask, encoder_answer_relevance_atten, use_cache, **kwargs):
        ...
        return {
            ...
            "decoder_input_ids": decoder_input_ids,
            "attention_mask": attention_mask,
            "encoder_answer_relevance_atten": encoder_answer_relevance_atten, # add to the generation process
